// ignore_for_file: must_be_immutable, prefer_const_constructors,ignore_for_file:, use_key_in_widget_constructors, library_private_types_in_public_api, no_leading_underscores_for_local_identifiers, prefer_typing_uninitialized_variables, library_private_types_in_public_api, library_private_types_in_public_api, library_private_types_in_public_api, library_private_types_in_public_api
...
class _DailyGraphState extends State<DailyGraph> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.hourlyList != null && widget.hourlyList!.isNotEmpty) {
      data = widget.hourlyList!.map((entry) {
        return _ChartData(
          'H ${entry.hour}',
      ...
      data = widget.dailyList!.map((entry) {
        return _ChartData(
          entry.day,
          entry.steps.toDouble(),
          entry.calories,
      ...
      data = widget.weeklyList!.asMap().entries.map((entry) {
        final int index = entry.key;
        final Weekly weekly = entry.value;
        return _ChartData(
          'Week ${index + 1}',
      ...
      data = widget.monthlyList!.asMap().entries.map((entry) {
        final Monthly monthly = entry.value;
        return _ChartData(
          monthly.name,
      ...
      data = widget.yearlyList!.asMap().entries.map((entry) {
        final Yearly yearly = entry.value;
        return _ChartData(
...
class GraphWidget extends StatefulWidget {
  ...
  GraphWidget(this.weekData, this.currentLanguage, {super.key});
...
class _GraphWidgetState extends State<GraphWidget> {
  ...
  @override
  Widget build(BuildContext context) {
    List<ChartData> data = widget.weekData.asMap().entries.map((entry) {
      final int index = entry.key;
